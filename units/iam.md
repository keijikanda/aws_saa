...
- **IAMポリシー（アイデンティティベースポリシー）**:
  - IAM ユーザー / グループ / ロールにアタッチするポリシー。
  - 「この主体が何をできるか」を定義する。
  - 例: `AppRole` に `s3:GetObject` を許可するポリシーを付与。
- **リソースベースポリシー**:
  - S3 バケット、SQS キュー、SNS トピック、KMS キー、Lambda などのリソース側に付けるポリシー。
  - 「このリソースに誰がアクセスできるか」を定義する。
  - 例: バケットポリシーで別アカウントのロールに `s3:GetObject` を許可。
- **使い分けの要点**:
  - 同一アカウント内アクセスでは IAM ポリシー中心で設計することが多い。
  - クロスアカウントアクセスでは、呼び出し元の IAM ポリシーと、呼び出し先のリソースベースポリシーの両方を確認する。
  - 明示的 Deny は Allow より常に優先される。
  - すべての AWS サービスがリソースベースポリシーに対応しているわけではない。
